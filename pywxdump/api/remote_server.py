...
from .utils import error9999, gc, asyncError9999, get_db_time
...
@rs_api.post('/roomInfo')
@error9999
def roomInfo(wxid: str = Body(None, embed=True)):
    """
    获取联系人的聊天记录数量
    :return:
    """
    ...
    db_config = gc.get_db_config()
    db = DBHandler(db_config, my_wxid=my_wxid)
    room = db.get_all_room(wxid)
    return ReJson(0, room)
...
@rs_api.api_route('/get_db_time', methods=["GET", 'POST'])
@error9999
def get_readme():
    my_wxid = gc.get_conf(gc.at, "last")
    if not my_wxid: return ReJson(1001, body="my_wxid is required")
    wx_path = gc.get_conf(my_wxid, "wx_path")
    wx_id = gc.get_conf(my_wxid, "my_wxid")
    code, res = get_db_time(wx_path, wx_id)
    if code:
        return ReJson(0, body=res)
    else:
        return ReJson(2001, body="status_code is not 200" + res)
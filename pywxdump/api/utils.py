...
def get_db_time(wx_path, wx_id):
    # 获取当前正在使用的数据库
    try:
        with open(wx_path + "\\Msg\\Multi\\config.ini", "r",
                  encoding="utf-8") as f:
            database = f.read()
            logging.info(f'当前使用数据库 {database}')
            from pathlib import Path
            import datetime
            # 创建Path对象
            file_path = Path(wx_path + '\\Msg\\Multi\\' + database)
            ...
            logging.info(f"文件最后修改时间: {dt}")
            return True, dt.strftime("%Y-%m-%d %H:%M:%S")
    except Exception as e:
        logging.error(f"获取数据库时间失败: {e}")
        return False, str(e)
